...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, WIoU=False, ShapeIoU=False,EfficiCIoULoss=False,
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU or mpdiou or WIoU or EfficiCIoULoss:
...
# def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False,HIoU = False ,EIoU=False, SIoU=False, ShapeIoU=False, EfficiCIoULoss=False, eps=1e-7, scale=0.0):
#     """
#     Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
#
#     Args:
...
#     # IoU
#     iou = inter / union
#     if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU or EfficiCIoULoss or HIoU:
#         cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
#         ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
#         if CIoU or DIoU or EIoU or SIoU or HIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
#             c2 = cw ** 2 + ch ** 2 + eps  # convex diagonal squared
#             rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4  # center dist ** 2
#             if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
#                 # v--长宽比一致性度量
#                 v = (4 / math.pi ** 2) * (torch.atan(w2 / h2) - torch.atan(w1 / h1)).pow(2)
#                 with torch.no_grad():
#                     alpha = v / (v - iou + (1 + eps)) # 动态调节alpha
#                 # # 长宽比差异性度量
#                 # d = torch.exp(torch.abs(torch.log(w1 / h1)-torch.log(w2 / h2)))
#                 # # with torch.no_grad():
#                 # beta = d / (d - iou + (1 + eps)) # 动态调节beta
#                 #
#                 # # 尺度不变性度量
#                 # s = torch.exp(torch.abs(torch.log(w1 / w2) + torch.log(h1 / h2)))
#                 # with torch.no_grad():
#                 #     gamma = s / (s + (1 - iou) + eps)
#
#                 return iou - (rho2 / c2 + v * alpha)
#                 # return iou - (rho2 / c2 + v * alpha +  d * beta)  # CIoU
#                 # return iou - (rho2 / c2 + v * alpha + s * gamma)
#             elif HIoU:
#                 rho2_w = (torch.log(w2) - torch.log(w1)) ** 2
#                 rho2_h = (torch.log(w2) - torch.log(w1)) ** 2
#                 l_asp = (torch.log(1 + rho2_w) / (5 * torch.log(1 + cw ** 2))) + (
#                             torch.log(1 + rho2_h) / (5 * torch.log(1 + ch ** 2)))
#
#                 s_cw = (b2_x1 + b2_x2 - b1_x1 - b1_x2) * 0.5 + eps
#                 s_ch = (b2_y1 + b2_y2 - b1_y1 - b1_y2) * 0.5 + eps
#                 sigma = torch.pow(s_cw ** 2 + s_ch ** 2, 0.5)
...
#                 angle_cost = torch.cos(torch.arcsin(sin_alpha) * 2 - math.pi / 2)
#
#                 return iou - (rho2 / c2 + l_asp + 0.5 * angle_cost)
#             elif EIoU:
#                 rho_w2 = ((b2_x2 - b2_x1) - (b1_x2 - b1_x1)) ** 2
#                 rho_h2 = ((b2_y2 - b2_y1) - (b1_y2 - b1_y1)) ** 2
#                 cw2 = cw ** 2 + eps
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                omiga_h = ww * torch.abs(h1 - h2) / torch.max(h1, h2)
                shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
                return innner_iou - distance - 0.5 * shape_cost #shapeIou
...
def bbox_focaler_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, ShapeIoU=False, eps=1e-7, scale=0.0, d=0.0, u=0.95):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
