...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max, use_dfl=False):
        ...
        if self.use_wiseiou:
            self.wiou_loss = WiseIouLoss(ltype='CIoU', monotonous=None, inner_iou=False, focaler_iou=False)

    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask, hw, mpdiou_hw=None):
        """IoU loss."""
        ...
        if self.use_wiseiou:
            ...
            # iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False,  HIoU=True, CIoU=False,)
            # iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=False, SIoU=True)
            # iou = bbox_inner_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=False, ratio=0.7)
            ...
            # iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, EfficiCIoULoss=True)
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask],
                           ...
                           ShapeIoU=False, hw=hw[fg_mask], mpdiou=False, Inner=False, Focaleriou=True,EfficiCIoULoss=True,
