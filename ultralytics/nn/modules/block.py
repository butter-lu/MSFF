...
__all__ = ('DFL', 'HGBlock', 'HGStem', 'SPP', 'CSPM', 'C1', 'C2', 'C3', 'C2f', 'C3x', 'C3TR', 'C3Ghost',
           'GhostBottleneck', 'Bottleneck', 'BottleneckCSP', 'Proto', 'RepC3','ILCB')
...
class CSPM(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.m1(x1)
        x3 = self.m2(x2)
        y1 = self.cv6(self.cv5(torch.cat((x1, x2, x3, self.m3(x3)), 1)))
        y2 = self.cv2(x)
...
class ILCB(nn.Module):
    def __init__(self, c1, c2):
        super(ILCB, self).__init__()
        # 定义全连接层
